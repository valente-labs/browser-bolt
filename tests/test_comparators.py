...
KEYS = {"OPENROUTER_API_KEY": "offline-router", "CEREBRAS_API_KEY": "offline-cerebras"}
...
def test_profiles_pinned_and_do_not_mutate_environment(page, name):
    ...
    with client_for(payload) as client:
        ...
        expected = model.TYPESAFE_OPENROUTER_MODEL if paired else comparators.CHAT_MODELS[name]
        assert call["model"] == result["model"] == expected
        assert call["provider"] == ("cerebras" if name == "qwen" else "openrouter")
...
@pytest.mark.parametrize("name", ["jev_qwen", "jev_astra", "jev_opus"])
